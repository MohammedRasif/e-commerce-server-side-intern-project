...
async function run() {
  try {
    ...
    const oderProductCollection = client.db("oderDB").collection("oderProduct");

    //await client.connect();
    // Send a ping to confirm a successful connection
    //await client.db("admin").command({ ping: 1 });

    app.post("/oderProduct", async (req, res) => {
      const newOderProduct = req.body;
      console.log(newOderProduct);
...
app.get("/", (req, res) => {
  res.send("e-commerce side is running");
});
app.listen(port, () => {
